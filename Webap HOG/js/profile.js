...
const Profile = {
    init() {
        ...
        // Быстрый переключатель языка в шапке (дублирует настройку из профиля)
        document.getElementById("lang-toggle").addEventListener("click", () => {
            haptic("light");
            setLang(App.state.lang === "ru" ? "en" : "ru");
        });
        ...
        const uid = App.params.uid || App.tg?.initDataUnsafe?.user?.id;
        ...
        // VIP-плашка: статус должен быть виден, иначе он не мотивирует занести больше
        const vipCard = document.getElementById("vip-card");
        if (vipCard) vipCard.hidden = !App.params.vip;
    ...
    syncLangSeg() {
        ...
        const tgl = document.getElementById("lang-toggle-text");
        if (tgl) tgl.textContent = App.state.lang.toUpperCase();
    },
    ...
    render() {
        ...
        document.getElementById("prof-streak").textContent = Quests.streak.n || 0;
        document.getElementById("prof-best").textContent = Quests.streak.best || 0;
        document.getElementById("profile-level").textContent =
            t("level") + " " + Quests.level();

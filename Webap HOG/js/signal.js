...
const Signal = {
    ...
    // Доводим карточку сигнала в экран целиком. Простой scrollIntoView прижимал
    // её верх к границе, и кнопка «Открыть сделку» оставалась под таб-баром —
    // приходилось доскроливать руками ровно перед тем действием, ради которого
    // всё и затевается. Считаем видимую область (без фиксированного таб-бара)
    // и двигаем ровно настолько, насколько нужно.
    scrollToSignal() {
        const card = this.el("signal-card");
        if (card.hidden) return;
        const tabbar = document.querySelector(".tabbar");
        const guard = (tabbar ? tabbar.offsetHeight : 72) + 10;
        const visible = window.innerHeight - guard;
        const rect = card.getBoundingClientRect();

        let delta = 0;
        if (rect.height <= visible) {
            if (rect.bottom > visible) delta = rect.bottom - visible;
            else if (rect.top < 8) delta = rect.top - 8;
        } else {
            delta = rect.top - 8; // не влезает целиком — показываем с начала
        }
    ...
    showResult() {
        ...
        // Ждём окончания анимации появления: во время неё карточка едет по Y,
        // и замер для прокрутки съезжает
        setTimeout(() => this.scrollToSignal(), 380);

...
const translations = {
    ...
    ru: {
        ...
        history_title: "Последние сигналы",
        // --- КОНЕЦ НОВОГО ---
    },
};
...
// --- Хранилище состояний таймеров (независимые для standard и otc) ---
const timerStates = {
    standard: { timerId: null, secondsLeft: 60 },
...
function switchLanguage(lang) {
    ...
    // Перерисовать историю сигналов на новом языке
    renderSignalHistory();
...
function initTelegramWebApp() {
    if (!tgApp) return; // Открыто в обычном браузере — работаем как раньше
    try {
        tgApp.ready();
        tgApp.expand(); // Полноэкранный режим внутри Telegram
        // Если язык ещё не выбирали вручную — берём язык аккаунта Telegram
        if (!localStorage.getItem("selectedLanguage")) {
            const code =
                (tgApp.initDataUnsafe &&
                    tgApp.initDataUnsafe.user &&
                    tgApp.initDataUnsafe.user.language_code) ||
                "";
            localStorage.setItem(
                "selectedLanguage",
                code.toLowerCase().startsWith("ru") ? "ru" : "en",
            );
        }
    } catch (e) {
...
function hapticFeedback(style) {
    try {
        if (tgApp && tgApp.HapticFeedback) {
            tgApp.HapticFeedback.impactOccurred(style || "medium");
        }
    } catch (e) {
        /* вибрация недоступна — не критично */
    }
}

// --- История последних сигналов (localStorage, только на устройстве юзера) ---
...
function saveSignalToHistory(pair, type, time) {
    const history = loadSignalHistory();
    history.unshift({ pair: pair, type: type, time: time });
    if (history.length > SIGNAL_HISTORY_MAX) {
        history.length = SIGNAL_HISTORY_MAX;
    }
    try {
        localStorage.setItem(SIGNAL_HISTORY_KEY, JSON.stringify(history));
    } catch (e) {
        /* localStorage переполнен/недоступен — просто не сохраняем */
    }
    renderSignalHistory();
}
...
function renderSignalHistory() {
    ...
    history.forEach((item) => {
        ...
        pairSpan.className = "history-pair";
        pairSpan.textContent = item.pair;

        const actionSpan = document.createElement("span");
        actionSpan.className = "history-action " + item.type;
        actionSpan.textContent =
            item.type === "buy"
                ? translations[lang]?.signal_buy || "BUY ⬆"
                : translations[lang]?.signal_sell || "SELL ⬇";
...
document.addEventListener("DOMContentLoaded", function () {
    // --- Telegram WebApp: expand, язык из аккаунта (до загрузки языка!) ---
    initTelegramWebApp();
    ...
    function getSignal() {
        ...
        setTimeout(() => {
            ...
            // Вибрация в Telegram + запись в историю сигналов
            hapticFeedback("medium");
            saveSignalToHistory(pair, randomSignalType, now.toLocaleTimeString());
    ...
    // --- Обновить ось Y с реальными ценами и сеткой с квадратными ячейками ---

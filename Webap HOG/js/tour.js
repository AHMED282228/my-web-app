...
const Tour = {
    ...
    showStep() {
        ...
        // Гасим прожектор НЕМЕДЛЕННО, если у шага нет цели или мы уходим на другую
        // вкладку. Иначе кольцо ещё 0.38 с висит по координатам прошлого экрана —
        // на финальном шаге оно приходилось ровно на карточку «Валютная пара»
        // и выглядело так, будто подсветка переехала с «Рынка» на главный экран.
        if (!step.target || viewChanged) this.place(null);

        // После смены вкладки ждём завершения её анимации, иначе замер съедет
        const delay = viewChanged ? 380 : 20;
        setTimeout(() => {
            if (!this.active) return;
            const target = step.target && document.querySelector(step.target);
            // center стабильнее «nearest»: элемент не липнет к краю под шапкой
            if (target) target.scrollIntoView({ block: "center", inline: "nearest" });
            this.placeWhenSettled(target);
        }, delay);
    ...
    // Перемеряет позицию цели, пока она не перестанет двигаться: скролл в webview
    // Telegram докручивается асинхронно, и разовый замер ставит прожектор по старым
    // координатам (кривая обводка). place() двигает прожектор по CSS-transition,
    // поэтому промежуточные вызовы просто плавно доводят его до финала.
    placeWhenSettled(target) {
        const token = ++this._placeToken;
        let tries = 0;
        let lastTop = null;
        const tick = () => {
            if (!this.active || token !== this._placeToken) return;
            this.place(target);
            const top = target
                ? Math.round(target.getBoundingClientRect().top)
                : 0;
            if (++tries < 14 && top !== lastTop) {
                lastTop = top;
                setTimeout(tick, 40);
            }
        };
